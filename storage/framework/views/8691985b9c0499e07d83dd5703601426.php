<?php $__env->startSection('title', __('movies.browse')); ?>
    ...
    <div id="heroContent" style="position:absolute;inset:0;z-index:3;display:flex;align-items:center;padding:0 4%;pointer-events:none;">
        <div style="max-width:580px;pointer-events:all;">
            ...
            <div style="display:flex;gap:12px;flex-wrap:wrap;">
                <a id="hLink" href="#" class="btn btn-primary"><i class="fas fa-info-circle"></i> <?php echo e(__('movies.more_info')); ?></a>
                <button id="hFavBtn" class="btn btn-secondary" onclick="heroFavToggle()"><i class="far fa-heart"></i> <?php echo e(__('favorites.add')); ?></button>
            </div>
        </div>
    </div>
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;width:100%;align-items:center;">
        <div style="flex:1;min-width:220px;position:relative;">
            <input
                type="text"
                id="searchInput"
                placeholder="<?php echo e(__('movies.search_placeholder')); ?>"
                autocomplete="off"
        ...
        <select id="typeFilter" class="filter-select">
            <option value=""><?php echo e(__('movies.type_all')); ?></option>
            <option value="movie"><?php echo e(__('movies.type_movie')); ?></option>
            <option value="series"><?php echo e(__('movies.type_series')); ?></option>
        </select>
        <input type="text" id="yearFilter" placeholder="<?php echo e(__('movies.year_placeholder')); ?>"
        ...
        <button onclick="doSearch()" class="btn btn-primary" style="padding:10px 20px;">
            <i class="fas fa-search"></i> <?php echo e(__('movies.search')); ?>
...
<div id="searchResultsSection" style="display:none;padding:0 4%;margin-bottom:40px;">
    ...
    <div style="text-align:center;padding:32px 0;">
        ...
        <div id="endMessage" style="color:var(--text-muted);font-size:0.85rem;display:none;"><?php echo e(__('movies.end_of_results')); ?></div>
    </div>
...
<div id="categoriesSection">
    <section class="row-section" id="cat-trending">
        <h2 class="row-title"><?php echo e(__('movies.trending')); ?></h2>
    ...
    <section class="row-section" id="cat-action">
        <h2 class="row-title"><?php echo e(__('movies.action')); ?></h2>
    ...
    <section class="row-section" id="cat-drama">
        <h2 class="row-title"><?php echo e(__('movies.drama')); ?></h2>
    ...
    <section class="row-section" id="cat-comedy">
        <h2 class="row-title"><?php echo e(__('movies.comedy')); ?></h2>
...
const favoriteIds = new Set(<?php echo json_encode($favoriteIds, 15, 512) ?>);
...
function goToSlide(idx) {
    ...
    document.getElementById('hBadge').innerHTML = `
        ...
        <span style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);color:#aaa;padding:4px 10px;border-radius:4px;font-size:0.72rem;font-weight:600;">
            #${heroIndex + 1} <?php echo e(__('movies.trending')); ?>
    ...
    // Plot
    const plot = (m.Plot && m.Plot !== 'N/A') ? m.Plot : '<?php echo e(__("movies.hero_loading")); ?>';
    ...
    document.getElementById('hCast').innerHTML = actors
        ? `<i class="fas fa-users" style="margin-right:6px;color:#888;"></i><span style="color:#888;"><?php echo e(__("movies.cast")); ?>:</span> ${actors}`
        : '';
...
function updateHeroFavBtn() {
    ...
    btn.innerHTML = `<i class="${isFav ? 'fas' : 'far'} fa-heart"></i> ${isFav ? '<?php echo e(__("favorites.remove")); ?>' : '<?php echo e(__("favorites.add")); ?>'}`;
}
...
function doSearch() {
    ...
    if (hasFilter) {
        ...
        // Build label
        let label = '<?php echo e(__("movies.search_results")); ?>';
        const parts = [];
        if (currentQuery) parts.push('"' + currentQuery + '"');
        if (currentType)  parts.push('<?php echo e(__("movies.type")); ?>: ' + currentType);
        if (currentYear)  parts.push('<?php echo e(__("movies.year")); ?>: ' + currentYear);
        if (parts.length) label = '<?php echo e(__("movies.results_for")); ?>: ' + parts.join(' · ');
    ...
    try {
        ...
        if (searchPage === 1) {
            ...
            let titleLabel = currentQuery
                ? '<?php echo e(__("movies.results_for")); ?>: "' + currentQuery + '" (' + total + ' <?php echo e(__("movies.results")); ?>)'
                : '<?php echo e(__("movies.results")); ?>: ' + total + ' <?php echo e(__("movies.results")); ?>';
        ...
        if (!movies.length && searchPage === 1) {
            grid.innerHTML = `
                <div class="empty-state" style="grid-column:1/-1">
                    ...
                    <h3><?php echo e(__('movies.no_results')); ?></h3>
                    <p><?php echo e(__('movies.no_results_desc')); ?></p>
                </div>`;
            searchHasMore = false;
        } else {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\bubub-movie-baru\resources\views/movies/index.blade.php ENDPATH**/ ?>